...
1. サービス発案者（起業家）が LaunchHub 上でプロジェクトの作成とタスクの登録を行います。タスクはプロジェクトの目的を達成するために必要なタスクを洗い出す必要があり、chatGPT がその洗い出しを支援します。各タスクには達成時の報酬として渡す SBT（SoulBound. Token・譲渡不可能な NFT）が設定されています。SBT はサービス収益の分配を受け取る権利です。タスク報酬として給与ではなく、収益分配権利を与えるのがこのサービスの最大の特徴です。

2. 貢献者（労働者）がタスクを実行し、タスクのアウトプットが発案者に認められれば、報酬としてタスクに定められた SBT が配布されます。

3. サービスが売上を立てた際、その売上の一部は発案者にわたり、残りの収益に関しては SBT 保有量に応じて貢献者に分配されます。このロジックはコントラクトに定められたものであり、発案者や LaunchHub をトラストする必要なく自動執行されます。
...
プロジェクトの発案者は、そのプロジェクト専用の SBT コントラクトと金庫コントラクトをガスレスでデプロイします。その後、貢献度に応じてメンバーに SBT を配布し、プロジェクトの売上が SBT の保有割合に応じて自動で分配されるようになっています。
...
| ----------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| プロジェクトのファウンダーがプロジェクトの SBT コントラクトをデプロイした際の hash  | "0x3ed277207cd2f852baeafa53c33da164e691b862898a34c0302437d830e5f8d0" |
| プロジェクトのファウンダーがプロジェクト用の金庫コントラクトをデプロイした際の hash | "0xa6e33725ed4a3de29864c995efac62bbd790b678a969cd3dec66c63a36571d9c" |
| メンバーに SBT を配布した際の hash                                                  | "0x88d32f2592e59592bded0d937679c6446fbfcf4290b753a4bab04fb0ad56274c" |
| 配分された収益を引き出した際の hash                                                 | "0x0b0c0e0829be51f606ba88743ff4cd8ecbbd02b5d57b7320dfae93e3eda83fc4" |

各トランザクションでは AA の技術（ERC-4337）が用いられているため、上記のトランザクションの from と to は通常のトランザクションとは異なり、今回使用したアカウントのアドレスが表示されません。
